...
- `recipe` — 에이전트 협업 도구 모음: Advisor 감독 워크플로(위임·검증), typed 스킬 기반 워크플로 정의·컴파일(skill·pipeline), 외부 에이전트 CLI 원샷 실행 래퍼(oneshot), codex-review CI 시크릿 재시드(codex-auth-reseed)를 제공합니다.
