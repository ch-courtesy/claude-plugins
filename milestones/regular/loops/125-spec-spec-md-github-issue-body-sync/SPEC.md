...
2. 시스템은 Issue body를 [자리표시 2줄 + `<!-- autopilot:spec-sync:begin -->` fence + `## SPEC.md (auto-synced)` heading + SPEC.md 전문 + `<!-- autopilot:spec-sync:end -->` fence] 구조로 유지한다.
...
- 자리표시 2줄 보존·HTML 코멘트 fence 구조·SPEC 전문 append·실패 처리·self-referential 규약·현재 호출 면제 예외 모두 문서화
...
- Issue body update는 `gh issue edit <id> --body-file <tempfile>` (GitHub CLI) 방식을 사용한다 — SPEC.md 전문이 멀티라인·frontmatter·backtick을 포함하므로 인라인 `--body "..."`는 셸 파싱 실패 위험이 있다. step 2의 기존 `gh` 호출 실패 처리 규약과 일관하게 abort하며 roll-back은 수행하지 않는다.
