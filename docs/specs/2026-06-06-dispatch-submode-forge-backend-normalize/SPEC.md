...
- forge 서브모드에서 PR 이 없으면(생성 누락·상태 손상) 머지를 진행하지 않고 차단한다(비완료
  종착) — approver 게이트 제거로 사라진 "PR 없음" 암묵 가드를 명시적으로 대체해, PR 없이 대상
  브랜치에 직접 머지하는 것을 막는다. direct 서브모드(PR 없이 동작하는 계약)에는 적용하지 않는다.
