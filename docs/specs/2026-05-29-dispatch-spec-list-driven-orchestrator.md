...
## 제약 (있을 때만)
- **scope.include 안 self-modifying 명시 허용**: 이 SPEC은 dispatch가 자기 SKILL.md·driver·내부 참조 문서를 수정·삭제·재작성하는 것을 본 SPEC의 주된 산출물로 정의한다. `scope.include`에 열거된 파일들은 본 SPEC이 명시적으로 수정 권한을 부여한다. 자율 실행기 헌법 §절대 금지의 "설계·명세 문서 수정 금지"는 `scope.include` 밖 파일에만 적용되는 것으로 해석한다. cold-start 정합성은 매 이터 시작 시 노트·git history·디스크 상태 점검으로 확보한다.
...
## 위험 (있을 때만)
- **self-modifying 콜드 스타트 정합성**: dispatch가 자기 SKILL.md·driver를 재작성하는 이터의 결과가 다음 이터 콜드 스타트에서 노트·디스크와 어긋날 수 있다. 매 이터 종료에 노트 인계 메모를 명시하고, 다음 이터 시작에 디스크 상태·노트·git log를 모두 읽어 정합성을 확인한다.
