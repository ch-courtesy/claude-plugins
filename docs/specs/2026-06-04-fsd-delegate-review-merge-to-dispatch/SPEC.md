...
12. `bash plugins/autopilot/skills/fsd/references/poll.sh selftest`가 ALL PASS 하고, "dispatch run 미완 → 전진 없음·상태 불변", "dispatch run 전체 머지 종착 → task done 전이" 케이스를 포함하며 외부 승인 대기 케이스는 포함하지 않는다.
...
- fsd 자체 forge/머지 제거 후 dispatch가 머지를 수행하지 못하는 환경(forge 미구성·direct 불가)에서는 task가 done에 이르지 못할 수 있다 — poll은 멈추지 않고 멱등 재드레인하며 상태를 바꾸지 않는다(완료 조건 5). dispatch direct 서브모드는 forge 없이도 ff-only 직접 머지하므로 일반 환경에서 자율 종착한다.
