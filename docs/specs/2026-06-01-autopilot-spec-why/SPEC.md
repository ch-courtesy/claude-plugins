...
## 목적 (왜)
<!-- 1–3문장. 완료 조건의 종속 앵커일 뿐 검증 기준이 아니다. -->
autopilot:spec이 발행하는 SPEC에서 변경의 WHY(왜)가 휘발되고 WHAT(완료 조건)만 남는 문제를 풀기 위함이다. 목적을 1급 산출물로 보존하면 구현 모델이 *글자는 맞지만 취지가 어긋나는* 구현을 피하고, SPEC에 안 적힌 갈림길에서 의도한 방향으로 fallback 추론을 하며, "맞는 WHAT, 틀린 목적"을 잡는 정합 검사축이 생긴다. 이 SPEC 문서 자체도 자신이 신설하는 목적 섹션 계약을 시연한다.
