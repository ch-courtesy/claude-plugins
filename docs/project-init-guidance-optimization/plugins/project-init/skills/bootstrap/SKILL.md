...
2. **`CLAUDE.md` 생성.** 없을 때만 `skills/bootstrap/CLAUDE.md`를 루트에 복사합니다. 프로젝트 세부 정보는 `rules/`로 분리합니다.
...
3. **카테고리 선택.** 부모 `skills/`에서 `*-rule-creator/`만 열거해 접미사를 제거합니다. 하드코딩하거나 다른 경로를 탐색하지 않습니다. 남은 항목만 추가할 때는 기존 카테고리를 제외합니다.
   ...
   - **아무것도 생성하지 않음** — `CLAUDE.md`만 두고 5단계로 이동합니다. 나중에 개별 `*-rule-creator`로 추가 가능함을 알립니다.
