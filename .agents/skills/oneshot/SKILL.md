...
outputs:
  ...
  signals: {type: array, description: 이번 실행에서 meta_dir/signals 에 남은 파일명 (매 실행 초기화됨)}
...
3. stdout 의 JSON 을 outputs 스키마와 대조해 그대로 보고한다. 실패해도 stdout 은 JSON 이므로(`error` 필드) 파싱은 언제나 성공한다. 종료 여부는 판정하지 않는다 — `signals`·`commits`·`exit_code`는 호출자가 해석한다.
