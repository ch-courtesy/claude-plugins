...
`flow` 는 내장 **dynamic Workflow 도구가 미가용**인 환경에서도 그 오케스트레이션을 재현하는 **독립 범용 하니스**(Workflow Replica)의 진입점이다. 임의의 의존성 그래프(DAG)를 받아, 각 노드를 **그 의존성이 끝나는 즉시** 동시성 상한 안에서 실행하고(스트리밍 fan-out), 노드 결과를 후속 노드로 전달하며, 실패를 이행적으로 격리하고, 중단된 실행을 저널로 재개한다. 엔진은 **Python 표준 라이브러리만** 사용한다(외부 패키지·네트워크 없음). dispatch 등 다른 스킬이 strong-parallel 불가 환경의 폴백으로 프로그램적으로 호출할 수 있도록, 러너의 입력→구조화(JSON) 출력 계약은 자기완결적이다.
