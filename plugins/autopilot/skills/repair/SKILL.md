...
# repair
**REQUIRED RUNTIME CONTRACT:** `../../references/runtime-capabilities.md`를 읽고 질문, 계획 추적, 스킬 호출, 서브에이전트 사용을 현재 런타임 기능에 맞춘다.
...
호출 시 현재 런타임의 계획 추적 기능으로 단계를 등록한다. 모든 결정·승인은 런타임 계약의 구조화 질문 우선 규칙으로 받으며, 한 번에 한 주제만 다룬다.
...
완성된 SPEC 전체를 한 번 제시하고 런타임 계약의 구조화 질문 우선 규칙으로 단일 승인을 받는다. 발행 개수와 무관하게 **항상 `autopilot:dispatch` 를 추천**한다. 옵트인 핸드오프 규약은 `spec` 과 동일하다:
...
- 모든 선택은 런타임 계약의 구조화 질문 우선·직접 질문 폴백 규칙을 따른다. 한 번에 한 주제만 다루고 한 호출의 소문항은 최대 4개다.
