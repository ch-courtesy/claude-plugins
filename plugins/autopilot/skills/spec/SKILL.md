...
상위 자율 오케스트레이터(`autopilot:fsd` 등)가 호출하는 맥락에서는 step 7 의 옵트인 핸드오프 프롬프트("구현까지 자동 진행할까요?")를 **생략**하고 곧바로 진행한다(아래 step 7 참조) — 별도의 인자는 필요 없다. 명확화 인터뷰·미해결 마커 가드·최종 SPEC 산출은 그대로 수행한다.
...
기본 원칙은 "후속 스킬을 자동 호출하지 않는다"이다. **유일하게 허용되는 예외**는 step 7의 **옵트인 자동 핸드오프**다 — 미해결 마커 없음 + 최종 SPEC 단일 승인 + 사용자의 명시 동의, 이 세 조건을 모두 충족할 때만 구현 스킬(`autopilot:dispatch`)을 호출한다. 상위 자율 오케스트레이터(`autopilot:fsd` 등)가 호출하는 맥락에서는 미해결 마커 없음만으로 사용자 대면 프롬프트 없이 진행한다(step 7 참조). 그 경계 밖에서는 어떤 후속 스킬도 자동 호출하지 않는다.
...
**자율 오케스트레이터 호출 맥락**: 상위 자율 오케스트레이터(`autopilot:fsd` 등)가 spec 을 호출하는 맥락에서는 step 7 의 사용자 대면 프롬프트(최종 단일 승인 `AskUserQuestion`과 "구현까지 자동 진행할까요?" 핸드오프 `AskUserQuestion`)를 **모두 생략**하고, 미해결 마커가 없으면 곧바로 진행한다 — 오케스트레이터(fsd)가 항상 자율이라 진행 결정을 소유하므로 그 확인은 중복이고, 완전자율 운영에는 사람 개입 지점이 없어야 한다. 별도의 신호 인자는 필요 없다(같은 오케스트레이터가 상위 스킬→spec 를 연달아 실행하므로 호출 맥락만으로 생략이 성립한다). 명확화 인터뷰·미해결 마커 가드·자체 검토·최종 SPEC 산출은 그대로 수행한다. 미해결 마커가 **남아 있으면** 자율 맥락이어도 진행하지 않고 자율 실행 차단 사실과 `--resume` 해결을 안내하고 종료한다(품질 게이트 보존). 자율 오케스트레이터 맥락이 아닌 사람 단독 호출이면 위 두 프롬프트를 그대로 띄운다(하위 호환 — 문서-only 검토 경로 보존).

이 옵트인 핸드오프(미해결 마커 없음 + 최종 승인 + 명시 동의, 또는 자율 오케스트레이터 맥락 + 미해결 마커 없음)는 "후속 스킬을 자동 호출하지 않는다" 원칙의 유일하게 허용되는 예외다. 그 경계 밖에서는 추천만 남기고 어떤 후속 스킬도 자동 호출하지 않는다.
...
- 후속 스킬을 자동 호출하지 않는다. **유일 예외**: step 7 옵트인 핸드오프 — 미해결 마커 없음 + 최종 단일 승인 + 명시 동의를 모두 충족할 때(또는 자율 오케스트레이터 호출 맥락 + 미해결 마커 없음)만 `autopilot:dispatch`를 호출한다. 자율 오케스트레이터(`autopilot:fsd` 등) 맥락에서는 step 7 의 사용자 대면 프롬프트(최종 승인·핸드오프)를 생략한다(별도 인자 불필요).
