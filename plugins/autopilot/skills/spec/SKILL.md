...
task-id 형식 검증 실패 또는 자연어 입력 감지 시, 즉시 종료하지 않고 `AskUserQuestion`으로 사용자 의도를 분류하는 3옵션을 제시한다 (헌법 §11.6의 "후속 스킬 호출은 항상 AskUserQuestion 확인을 거쳐야 한다" 규칙과 동일 정신):
...
본 검증 실패 라우팅으로 spec의 책임 범위가 "task-id를 받아 SPEC 작성"에서 "사전 명확화 + task 생성/마일스톤 라우팅"으로 확장됐다. step 4 메커니즘 재사용으로 코드 중복은 피했으나 단일 책임 원칙이 약해질 수 있다 — 향후 별도 의도-탐색 스킬로 분리하는 것은 후속 task로 검토한다 (본 변경 범위 밖).
