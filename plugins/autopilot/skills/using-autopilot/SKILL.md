...
위 트리거가 **사용자가 요청한 새 코드 변경**을 라우팅한다면, 이 절은 **autopilot 자기 도구의 blocked·비정상**이라는 새 신호 유형을 받아 카테고리별 자가개선 행동으로 잇는다. **이 자가개선 정책의 단일 소유자는 `using-autopilot`이다** — 카테고리 enum·카테고리→행동 매핑·재귀 상한을 **여기서 단일 정의**하고, 실행 경로(단일 `execute-task` / 무인 `workflow-task` 드레인)와 무관하게 **항상 발동**한다(always-on). `execute-task`·`workflow-task`는 blocked의 `category`를 **표면화만** 하고(코드가 자동 발동하는 게 아니다), 그 category를 읽은 오케스트레이팅 세션이 아래 정책대로 행동한다(가이던스 성격). 사람이 매번 "버그인지 보고 고쳐라"라고 지시하지 않아도, 비정상을 그 자리에서 자가개선 신호로 인식한다. 이것은 기존 라우팅을 **대체·약화하지 않고 그 위에 가산**된다.

- **트리거** — `create-task`/`execute-task`/`workflow-task`(및 그 엔진 loop·forge·merge·review) 사용 중 blocked·비정상을 관찰했을 때: 잘못된/근거 약한 `blocked`, 오지 않을 상태를 무의미하게 대기, 모순된 상태 전이, 예기치 못한 실패·오보고 등. blocked 신호는 `category:` 필드를 싣는다(단일 경로: `execute-task` BLOCKED 로그/신호; 드레인 경로: `workflow-task` `failed_ids`와 함께).

- **카테고리 enum + 카테고리→행동 매핑** (단일 정의) — blocked `category`를 세 자가개선 행동 클래스로 가른다. 오분류를 줄이도록 판별 기준을 함께 둔다:
  - **`spec-gap`** — SPEC 수용기준 모호·scope 산출물 누락 등 **스펙 자체의 결함**(예: DoD가 요구하는 회귀 테스트 경로가 `scope.include` 밖). → **해당 SPEC scope 보정**(그 SPEC을 강화한다; 새 fix 태스크를 만들지 않는다). 단, 동반하는 도구 결함이 따로 있으면 그 부분만 별도 `tool-defect`로 등록한다.
  - **`tool-defect`** — 게이트·엔진 등 **autopilot 도구 자체의 결함**(헌법 BLOCKED 값 `config-gap`·`architecture-gap`이 여기에 해당). → **새 fix 태스크 등록**(`fix`가 정적 분석으로 진단해 본문=SPEC 작성 → `create-task`로 등록). 관찰 증상·재현 경로·결함 범위를 스펙에 담는다.
  - **`ops`** — push non-ff·stale 브랜치 등 **운영 아티팩트**(헌법 BLOCKED 값 `environment-gap`이 여기에 해당). → **정리**(브랜치/베이스 정합 등 운영 조치; **새 fix 태스크가 아니다**).
...
- **재귀 상한 (depth-1)** — 자가개선이 **생성한 fix 태스크**의 본문에는 `자가개선-비활성` 마커를 부여한다(본문 한 줄). 그 태스크가 다시 blocked돼도 **추가 자가개선을 트리거하지 않는다**(자기 수정의 자기 수정… 무한 재귀 방지, depth-1 상한). 단일 `execute-task`·무인 `workflow-task` 드레인 **양 경로가 이 마커를 존중**한다 — `자가개선-비활성` 마커가 있는 태스크의 blocked는 카테고리 매핑을 적용하지 않고 표준 blocked로 보고만 한다.
...
- **경계**
  ...
  - 무한 재귀는 위 **재귀 상한(depth-1)** 으로 차단하고, 한 발동은 **관찰된 구체적 비정상 1건**에 한정한다.
...
| "task 스킬이 이상하지만 일단 수동으로 우회하자" | autopilot 자기 도구의 blocked·비정상은 자가개선 신호다. 우회로 끝내지 말고 진단→`category`별 행동(「자가개선 정책」: spec-gap→SPEC 보정 / tool-defect→`fix` / ops→정리). 정당한 우회(일회성 환경·사용자 명시 지시)만 예외. |
| "사용자가 '이거 아님'·'아님'·'????'처럼 현재 단계를 비판했으니 흐름을 취소하자" | 흐름 중 사용자 부정 피드백은 흐름 종료 신호가 아니다. 현재 단계 실행 방식을 수정하고 흐름을 계속한다. |
