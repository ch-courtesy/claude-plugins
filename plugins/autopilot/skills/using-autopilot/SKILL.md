...
| "먼저 코드베이스를 둘러보고 / 코드를 파악·이해해야 시작하지" | 탐색·이해 필요는 라우팅의 예외가 아니다. 먼저 실행하고, 이해는 `feature`의 컨텍스트·명확화 단계에서 한다. |
...
`using-autopilot`은 **올바른 진입점(`feature`/`fix`)으로의 라우팅까지만** 책임진다. 작성(`feature` 인터뷰 / `fix` 정적 분석)→등록·상태 전이(`create-task` 소유)는 위 도식대로다. 하류(`execute-task`/`workflow-task`)는 오리엔테이션 표시일 뿐, 그 선택·기동·안내는 등록 이후 `create-task`의 책임이다. 정적 분석으로 해소 불가한 모호성(`[NEEDS CLARIFICATION]`)은 `feature` 인터뷰 재개로 완성한다.
