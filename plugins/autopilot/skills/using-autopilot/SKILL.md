...
`autopilot:spec`은 "어떤 작업이든 시작하기 전 가장 먼저 사용"하도록 일반화된 진입점이며, 위 신호들은 그 대표적 트리거다. spec의 명확화 인터뷰(집요함·결정 트리·추천 답·코드 우선)가 설계 탐색을 수행하므로, 별도의 사전 설계 단계 없이 spec이 곧 첫 단계다.
