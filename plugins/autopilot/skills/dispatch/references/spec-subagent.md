...
- 구현은 **반드시 `Skill(skill="loop", ...)`** 로만 한다 — loop 이 전용 격리 워크트리(`<spec_dir>/.worktree`)를 만들고 소유한다. 대상 파일을 **직접 편집하거나** 임의 작업 브랜치를 직접 만들어 구현하지 않으며, `loop.sh` 를 Bash 로 **직접 구동하지 않는다**(스킬로만). 워커의 cwd 워크트리(예: 세션·메인 체크아웃)를 점유하지 않는다.
