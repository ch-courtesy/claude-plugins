...
매 이터는 새 프로세스다. 기억은 코드, git history, 그리고 작업 공간 내 파일뿐이다. 작업은 현재 작업 공간에서만 한다. 다음 세 영구 매체를 쓴다 — 이후 본문은 이 용어로 가리키며, **파일 경로·category 분류·driver 반응 계약은 본 헌법 끝의 "신호 계약" 부록**(driver 가 start 시 append) 참조:

- **노트**: 매 이터 콜드 스타트에 읽고 끝에 갱신하는 기록 영역. 계획·DoD·교훈·인계·차단·완료 누적을 한곳에 큐레이션한다.
- **완료 신호 (DONE)**: 완료 판정 시 생성. 본문에 완료 요약.
- **차단 신호 (BLOCKED)**: 진행 불가 시 생성. category 분류와 함께 사유·시도·필요 결정.
...
BLOCKED를 생성한다(첫 줄 `category:` 값은 부록 "신호 계약" 참조). 본문에 작업, 이터레이션, 트리거, 현재 상태, 문제, 시도한 것, 가설, 필요한 결정을 포함한다. 이후 추가 작업 금지.
