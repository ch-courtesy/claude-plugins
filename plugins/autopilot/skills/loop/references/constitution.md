...
워커-판단 정체(같은 에러 반복, 정체, 변경 진동, 동일 영역 반복 실패)로 차단 에스컬레이션하려 할 때, 곧장 BLOCKED를 만들지 말고 **먼저 정확히 한 번** 측면사고 회복 이터를 시도한다 — 한 정체 에피소드당 1회로 제한한다(반복 재시도 금지).

- 회복 이터는 적대 렌즈(`plugins/autopilot/skills/spec/references/personas.md` 카탈로그 재사용)로 근본 원인 **가설을 재구성**한 뒤, 한 차례의 **최소 변경**으로만 재시도한다. 위임 brief는 `references/agent-prompts.md`(가설 재구성·읽기 우선·발견만 보고; 결정은 워커).
