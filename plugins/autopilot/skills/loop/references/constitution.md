...
- **시작 전**: SPEC.md (수용 기준·scope·verify) 이해됨? 메모리 파일(PLAN/NOTES/HANDOFF/RUN_LOG) 읽음? 이터 상한 인지?
- **이터 후**: 진전(§4.3 정의) 있음? 자기 분류 commit prefix? `fix:symptom` 누적 여부? 메모리 파일 갱신?
- **DONE 작성 전**: 4-Level Verifier(§3.4) + Self-Review 4축(§3.5) 모두 통과? 금지 행동(§7)·제1 원칙(§1) 위반 없음? scope 내 변경?
