...
- 정체성은 스펙 파일의 절대 경로다.
- 작업 위치는 스펙 디렉토리 아래 전용 워크트리(`<spec_dir>/.worktree`). 호출 위치와 무관하게 항상 새로 생성한다 — 보조 worktree 안에서 호출돼도 enclosing 워크트리를 재사용하지 않고 전용 워크트리를 새로 만든다. 정확한 경로는 `loop.sh paths <spec>`.
...
루프는 선택된 worker CLI를 무인 실행한다. Claude는 `--dangerously-skip-permissions`, Codex는 격리 worktree의 `--sandbox workspace-write`를 사용한다. 작업 공간에 secrets·`.env`·credentials·SSH key를 두지 말고 스펙에 secrets를 쓰지 않는다. 신뢰 못 한 외부 스펙은 받지 않는다.
...
└── .worktree/           # 작업 공간 (git worktree, info/exclude)
    ├── CLAUDE.md        # Claude worker용 헌법 복사본
    ├── AGENTS.md        # Codex worker용 헌법 복사본
    └── .loop/
        ├── BASE_SHA
...
- **에스컬레이션 전 측면사고(lateral) 회복**: 워커는 차단(BLOCKED) 발행 전에 정체 에피소드당 **정확히 1회** 측면사고 회복을 시도한다(진전이면 정상 루프, 무진전이면 표준 차단; 안전 직결 정지는 회복 없이 즉시). 새 드라이버 변수 없음 — 경계는 기존 정지 한계와 노트로만 보장. 계약·경계 SoT: `references/constitution.md §에스컬레이션 전 측면사고(lateral) 회복`.
