...
- **에스컬레이션 전 측면사고(lateral) 회복**: 워커-판단 정체로 차단하려 할 때, 워커는 BLOCKED 발행 전에 정체 에피소드당 **정확히 1회** 측면사고 회복 이터를 시도한다(진전이면 정상 루프, 무진전이면 표준 차단). 경계 — 안전 직결 정지(범위 이탈·테스트 약화·비밀 노출·평가/수용기준 편집)는 회복 없이 즉시 에스컬레이션하며, 회복은 드라이버가 강제하는 기존 정지 한계(증상-수정 연속·진동) 안에 머문다. 새 드라이버 변수는 없다 — 경계는 기존 정지 한계와 워커 메모리(노트)로만 보장된다. 계약 SoT: `references/constitution.md §에스컬레이션 전 측면사고(lateral) 회복`.
