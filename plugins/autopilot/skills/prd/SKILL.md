...
## Anti-Pattern: "이건 너무 단순해서 9단계 안 거쳐도 된다"

어떤 milestone도 9단계를 스킵하지 않는다. "이건 작은 milestone이라 step X 생략", "trivial하니 명확화 라운드 압축" 같은 합리화는 금지다 — "단순"해 보이는 milestone일수록 검증되지 않은 가정이 누적되어 dispatch가 분해 단계에서 헛돈다. 9단계의 출력 분량은 milestone 복잡도에 맞게 축약될 수 있지만(자유 산문 한두 문장도 허용), **단계 자체를 스킵하지 않는다**. step 4(접근법 비교)의 명시 OPT-OUT조차 step을 건너뛰는 게 아니라 "trivial이라 비교 안 함" 한 줄 명문화로 *통과*시키는 것이다.
...
UX·시각 콘텐츠가 무거운 milestone(예: 화면 흐름·다이어그램·레이아웃 비교)을 다룬다고 추정되면, `superpowers:brainstorming` 스킬의 visual companion을 1회 offer한다. **이 offer는 단독 메시지여야 한다** — 명확화 질문·요약·다른 콘텐츠와 결합 금지.

상세 가이드는 brainstorming 스킬의 `skills/brainstorming/visual-companion.md`를 참조한다. 본 스킬은 입구만 제공하며 메커니즘은 brainstorming의 것을 그대로 빌린다 (코드·운영 중복 회피).

offer 문구 (단독 메시지로 전송):
> "이 milestone에서 화면·다이어그램·레이아웃 같은 시각 콘텐츠를 다룰 수 있습니다. 원하시면 브라우저 기반 visual companion을 켜서 mockup·비교 이미지를 보여드릴 수 있어요. 토큰 사용량이 늘 수 있고 로컬 URL을 열게 됩니다. 켤까요?"

사용자 응답에 따라:
- 수락 → 이후 시각 질문은 brainstorming의 visual-companion.md 절차대로 처리
- 거절 → 텍스트 전용으로 계속
...
**YAGNI 강제 통과 (목표·범위 합의 직후, 1회)**: 섹션 3(목표·비전)과 섹션 5(범위.포함)가 사용자 승인을 받은 직후, 합의된 **각 항목**을 1회씩 강제 검토한다. 검토 형식:

> "[합의 항목 X]를 정말 본 milestone에 포함해야 합니까? 빼도 milestone 정의가 무너지지 않는다면 빼는 게 낫습니다."

각 항목마다 `AskUserQuestion`으로 (a) 유지 / (b) 제거 / (c) 별도 milestone으로 이관 — 3선택. 한 항목도 우회되지 않으며, 답변 후 "유지" 항목만 다음 단계로 넘긴다. 이 게이트는 본 step에서 **강제 1회** 실행되며, 사용자의 "다 유지" 일괄 응답도 항목별 명시 확인을 통과한 후에만 인정한다 — `AskUserQuestion`의 multi-select 한 화면 처리는 허용하되, 각 항목이 옵션으로 분리되어야 한다.
