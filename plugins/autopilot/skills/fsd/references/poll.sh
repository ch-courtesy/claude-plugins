...
#       · integrate 는 PR 미생성 상태에서만 (PR 이 차면 다음 드레인은 리뷰/머지 경로로).
#         또한 C2 ensure_pr 이 같은 head 의 open PR 을 재사용해 중복 PR 을 만들지 않는다.
#       · review  는 C3 의 head 가드로 같은 head 를 재수정하지 않는다.
#       · merge   는 Done 전이 후 종착 no-op (재머지 안 함).
#
# 이 단위는 C1~C4 의 공개 동작을 **조합**할 뿐, 각 스텝의 동작을 정의하지 않는다.
# 무거운 액션(start·integrate·review·merge)은 주입 가능한 명령으로 **서브프로세스**
...
poll_task() {
  ...
  local spec pr branch run_id cstate bstat
  ...
  # --- 2) 열린 승인 요청(PR) 보유 → 승인되면 머지(C4), 아니면 리뷰 자동수정(C3). ---
  if [[ -n "$pr" ]]; then
    if $POLL_MERGE_CMD approval "$pr" >/dev/null 2>&1; then
      ...
      pl_log "$id" "PR 열림·미승인 → review 자동수정(C3) pr=$pr"
      $POLL_REVIEW_CMD run "$id" "$spec" "$pr" "$branch" \
        || pl_log "$id" "review 라운드 종료/대기 pr=$pr"
    fi
    return 0
  fi
...
#!/usr/bin/env bash
echo "review $*" >> "$TRACE"   # run <id> <spec> <pr> <branch> — 대기 시뮬(상태 변화 없음)
exit 0
EOF
...
EOF
  ...
  # AC3 멱등: PR 이 찼으니 재드레인은 integrate 재호출/중복 PR 없이 리뷰 경로로.
  MOCK_TERM=done MOCK_APPROVED=0 poll_task g1 >/dev/null
  chk "AC3 멱등: integrate 중복 없음(총 1회)" "$(tcount 'forge integrate')" "1"
  [[ "$(tcount 'review run')" -ge 1 ]] && ok "AC3 재드레인은 리뷰 경로" || bad "AC3 재드레인은 리뷰 경로"

  # ---- AC2-c: PR 열림·미승인 → 리뷰 자동수정(C3) ----
  : > "$TRACE"
  ...
  [[ "$(tcount 'review run')" == "1" ]] && ok "AC2-c 미승인→review run" || bad "AC2-c 미승인→review run"
  [[ "$(tcount 'merge finish')" == "0" ]] && ok "AC2-c 미승인→머지 안 함" || bad "AC2-c 미승인→머지 안 함"
  # AC3 멱등: 동일 상태 재드레인 → 머지로 새지 않음, 상태 불변.
  MOCK_APPROVED=0 poll_task v1 >/dev/null
