...
#
# 출력: 한 줄 JSON  {status: persisted|skip, ...}
set -euo pipefail
...
if [[ -n "$ORIGIN_URL" ]]; then
  ...
  if command -v gh >/dev/null 2>&1; then
    PR_URL="$(gh pr create --base "$DEFAULT_BRANCH" --head "$PBC_BRANCH" \
              --title "chore(autopilot): persist task backend config SoT to main" \
              --body "백엔드 선택 SoT(\`$PBC_REL\`)를 메인에 영속화. config 파일 단독 변경." \
              2>/dev/null || gh pr view "$PBC_BRANCH" --json url -q .url 2>/dev/null || true)"
    gh pr merge "$PBC_BRANCH" --auto --merge 2>/dev/null \
      || gh pr merge "$PBC_BRANCH" --merge 2>/dev/null || true
  fi
