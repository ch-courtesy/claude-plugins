# 태스크 본문(=SPEC) 구조 (작성자 공용 — feature·fix)
...
본문 섹션. 백엔드 공통이며 정규 정의는 플러그인 `task-backend/contract.md`와 일치한다. `fix`는 공통 본문에
...
scope:
  include:
    - <변경 대상 glob>      # 작성 단계(feature: step 1 인터뷰 / fix: step 2 정적 진단)에서 식별한 변경
                            #   대상. 불명확하면 해당 모듈 디렉터리로 보수적으로 넓게(과좁은 include 는
                            #   정당한 변경을 halt 시킨다).
  exclude:
    - rules/**
    - milestones/**
...
## 무엇을 만들 것인가
<!-- WHAT/HOW 방어선: 무엇을 만들지(고칠지)만. 구현 방법·파일·라이브러리·클래스명은 제약으로 이동. -->

## 목적 (왜)
<!-- 이 변경을 왜 하는가 1–3문장. 완료 조건의 종속 앵커일 뿐 검증 기준이 아니다. 모호하면 [NEEDS CLARIFICATION: 왜 ...]. -->

## 진단
<!-- fix 전용 — 버그 태스크에만 포함한다. feature 본문에는 이 섹션이 없다. -->
### 증상
- 기대 동작: <무엇이 일어나야 하는가>
- 실제 동작: <무엇이 관찰되는가>
...
## 완료 조건
<!-- 5문장 패턴(항상 / …할 때 / …인 동안 / …이면(오류) / …기능이 켜지면)·언어 규칙은 ears-patterns.md.
     각 조건은 관찰 가능·독립 검증 가능. fix 는 "버그 미관찰 + 회귀 가드"를 인코딩한다. -->
...
- **frontmatter-first** — 본문 첫 줄은 `---`(여는 frontmatter). `scope.include` 는 작성 단계에서 식별한
...
- **완료 조건**은 `ears-patterns.md`의 5문장 패턴으로, 각 항목이 관찰 가능하고 독립적으로 테스트
  가능해야 한다(실행 스킬이 이걸로 검증).
- **진단 섹션(fix 전용)** — 근본 원인은 **확정이 아닌 가설**로 프레이밍하고 증거를 `파일:줄`로 싣는다.
  정적 분석 한정을 위반하는 실행·재현 가정을 적지 않는다.
