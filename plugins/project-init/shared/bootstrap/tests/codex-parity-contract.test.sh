...
check "project-init release surfaces are version 0.27.0" bash -c \
  "[ \"\$(jq -r .version '$CODEX_MANIFEST')\" = 0.27.0 ] && [ \"\$(jq -r .version '$CLAUDE_MANIFEST')\" = 0.27.0 ] && [ \"\$(jq -r '.plugins[] | select(.name == \"project-init\") | .version' '$CLAUDE_MARKETPLACE')\" = 0.27.0 ]"
