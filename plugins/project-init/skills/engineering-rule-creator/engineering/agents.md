...
| 항목 | 값 |
|---|---|
| 정의 위치 | `.claude/agents/orchestrator.md` (Claude Code 커스텀 에이전트) |
| 트리거 | 사용자가 자유 텍스트 요청으로 직접 호출 |
| 도구 권한 | 파일 읽기, 셸(git·gh 등 태스크 관리 CLI), Task(서브에이전트 호출) |
| 사용하는 서브에이전트 | `work-agent` (Task 도구의 `subagent_type`) |
| 코드 수정 권한 | 없음 (Edit/Write 미부여) |
...
- **트리 깊이 2 고정**: `parent → sub-태스크`. 그 이상의 깊이 금지. 더 깊이 필요해 보이면 깊이를 늘리지 말고 **parent 자체를 더 좁게 다시 정의**(가로로 풀기).
