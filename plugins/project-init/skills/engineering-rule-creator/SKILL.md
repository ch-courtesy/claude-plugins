...
5. **파일 기록 및 사후 작업.**
   ...
   - 이미 `rules/engineering/<sub>.md`가 있으면 **그대로 덮어쓰지 않습니다**. 새 본문과 기존 파일의 diff를 사용자에게 보여주고, 사용자가 **명시적으로 "덮어쓴다"·"교체한다"·"yes"** 등으로 응답한 경우에만 덮어씁니다. "확인했다"·"봤다"·침묵은 동의가 아닙니다 — 의심스러우면 묻지 않고 보존합니다.
