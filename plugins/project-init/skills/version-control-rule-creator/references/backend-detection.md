...
판별된 백엔드를 git 계열로 분류하는 **정적 매핑**으로 처리한다. 매핑에 없는 백엔드의 기본값은 **"git 계열 아님"**이므로, 향후 비-git 백엔드는 매핑에 넣지 않는 것만으로 동반 산출에서 자연히 빠진다. 매핑 멤버십(현재 `github`·`gitlab`)과 분류 판정의 **단일 출처는 `references/template_tools.py`의 `git-family` 서브커맨드**(`GIT_FAMILY` 집합)다 — 이 문서는 그 매핑을 설명할 뿐 멤버십 값을 따로 정의하지 않는다.
