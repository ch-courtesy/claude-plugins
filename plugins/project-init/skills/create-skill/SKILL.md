...
description은 에이전트가 **호출 여부를 판단하는 단일 출처**다.

- WHAT(무엇을 하는가) + WHEN(언제 쓰는가) + 트리거 키워드를 description 안에 모두 담는다.
...
description 완결성 원칙을 적용해 다음 세 요소를 결합한 description 초안을 작성한다.

- WHAT: 스킬이 하는 일을 동사-목적어로 요약
- WHEN: 활성화 조건을 "~할 때" 형태로 명시
- 키워드: 사용자가 실제로 쓸 법한 동의어·유사 표현 포함
