...
> 참고: `context-rule-creator/SKILL.md`와 거의 동일한 패턴이지만 입력 수집(placeholder 치환) 단계가 없으므로 step 번호가 1만큼 작습니다 — sibling의 step 5(파일 기록)는 본 SKILL.md의 step 4입니다.
...
2. **메타데이터 파싱.** 각 템플릿의 YAML frontmatter를 읽어 다음 필드를 사용합니다.
   ...
   - **`inputs` 필드는 이 스킬에서 지원하지 않습니다.** 템플릿 본문은 placeholder 치환 없이 그대로 복사되며, 사용자 입력이 필요한 부분은 `on_create`가 사용자에게 안내해 직접 채우게 합니다.
...
5. **사후 작업.** 템플릿의 `on_create` 지시를 그대로 수행합니다. 예시 (실제 작업은 `on_create`에서 정의):
...
- 단순 재실행으로 템플릿 선택을 바꾸지 않습니다 — 템플릿 변경은 사용자의 명시적 의도가 있을 때만.
