...
2. **벤더 선택 필수 질문.** 정확히 하나의 구조화된 사용자 질문 기능의 single-select 질문으로 설치할 벤더 골격 조합을 묻습니다.
   - `Claude Code + Codex (Recommended)`
   - `Claude Code only`
   - `Codex only`
   - 모든 선택지는 **최소 1개** 벤더를 포함해야 하며, 두 벤더를 모두 선택하는 구성을 권장합니다.
   - 구조화된 질문 기능이 빈 응답을 돌려주는 환경에서는 어떤 파일도 생성·변경하지 않고 같은 질문을 다시 제시합니다. 최대 **3회** 연속 무선택이면 아무 파일도 생성·변경하지 않은 채 bootstrap을 중단합니다.

3. **공통 선택 블록 질문.** 루트에 `AGENTS.md`가 없어 새로 생성할 때만, 카파시 코딩 룰과 사용자 상호작용 규칙 포함 조합을 정확히 하나의 구조화된 사용자 질문 기능의 single-select 질문으로 묻습니다.
   - `카파시 룰 + 사용자 상호작용 규칙 (Recommended)`
   - `카파시 룰만`
   - `사용자 상호작용 규칙만`
   - `둘 다 제외`
...
6. **카테고리 선택.** 부모 `skills/`에서 `*-rule-creator/`만 열거해 접미사를 제거합니다. 후보를 일반 메시지로 한 번 출력한 다음 구조화된 사용자 질문 기능으로 전체 생성, 일부 선택, 아무것도 생성하지 않음 중 하나를 받습니다. `일부 선택`이면 카테고리별로 `생성`/`건너뜀` single-select 질문을 반복해 원하는 카테고리만 수집합니다.
