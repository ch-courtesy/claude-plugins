...
# updated for the H1-normalize SPEC: the generated CLAUDE.md no longer carries a
# redundant "# CLAUDE.md" title; instead each assembled block begins with a
# sibling top-level (H1) heading so block boundaries are visible by heading level
# alone. Single-source assembly model:
#   - base CLAUDE.md template = always-included "카테고리별 지침" (now an H1 block)
#   - interaction-rules asset  = the optional "Claude 상호작용 규칙" (now an H1 block)
#   - karpathy-rules asset      = the optional 카파시 룰 (already an H1 block)
#   - SKILL.md documents the AskUserQuestion include/exclude branch and the
#     sibling-H1 boundary assembly (no longer anchored on a "# CLAUDE.md" title)
#
...
check "asset top heading is H1 '# Claude 상호작용 규칙'" grep -qxF '# Claude 상호작용 규칙' "$ASSET"
check "asset has NO H2 '## Claude 상호작용 규칙' anymore" bash -c "! grep -qxF '## Claude 상호작용 규칙' '$ASSET'"
...
#    and the redundant "# CLAUDE.md" title is gone.
check "base CLAUDE.md top heading is H1 '# 카테고리별 지침'" grep -qxF '# 카테고리별 지침' "$BASE"
check "base CLAUDE.md has NO H2 '## 카테고리별 지침' anymore" bash -c "! grep -qxF '## 카테고리별 지침' '$BASE'"
check "base CLAUDE.md has NO '# CLAUDE.md' title line" bash -c "! grep -qxF '# CLAUDE.md' '$BASE'"
...
check "base CLAUDE.md has exactly one H1" bash -c "[ \"\$(grep -cE '^# ' '$BASE')\" -eq 1 ]"
...
#    no longer anchored on a now-removed '# CLAUDE.md' title.
...
check "SKILL.md no longer anchors assembly on '# CLAUDE.md' title" bash -c "! grep -qF '\`# CLAUDE.md\`' '$SKILL'"
