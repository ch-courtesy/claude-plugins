...
**층이 분리돼 있다**: `oneshot`(raw 실행) ← `isolated-run`(격리 준비·검증을 맡는 에이전트 래퍼, typed 스킬) ← 파이프라인 `while`(반복·종료 판정). 도구는 재료를, 흐름은 판단을.
...
| `oneshot` | 외부 에이전트 CLI(claude·codex)를 **1회** 실행하는 raw 래퍼(`claude -p` 수준). 격리·커밋·반복·판정은 호출자가 정한다 — 격리까지 대신하는 에이전트 래퍼는 typed 스킬 `isolated-run` |
