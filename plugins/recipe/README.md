...
**층이 분리돼 있다**: `oneshot`은 실행만 하고 격리(작업 공간 준비)·커밋 여부·반복·종료 판정은 호출자가 정한다 — 도구는 재료를, 흐름은 판단을. 단 지시대로 실행할 수 없는 전제 위반(입력 파손·미지원 벤더·벤더 CLI 부재·cwd 이동 실패·지침 파일 못 읽음)은 도구가 스스로 중단한다.
