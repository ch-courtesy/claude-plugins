...
1. **컨텍스트 패키징 + 스폰.** 현재 런타임의 서브에이전트 기능으로 Advisor를 동기 생성한다. 플러그인 에이전트 등록을 지원하는 런타임이면 등록된 `recipe:advisor` 에이전트로 생성하고, 아니면 `agents/advisor.md`의 역할 정의 전문을 서브에이전트 프롬프트에 주입해 생성한다. 서브에이전트 기능 자체가 없으면 Advisor 감독이 불가함을 사용자에게 보고하고 Worker가 직접 수행한다. 스폰 프롬프트에 담는다:
   - 사용자 요청 원문 (가공 없이)
   - Worker가 이미 파악한 컨텍스트: 파일 경로, 프로젝트 컨벤션, 탐색 결과, 제약
   - 알려진 완료 기준 (테스트 명령 등)
2. **첫 응답 처리.** 응답 첫 줄의 상태 태그로 분기한다.
   - `BRIEF` → 3으로.
   - `QUESTION` → 질문을 가공 없이 사용자에게 전달(구조화 질문 기능이 있으면 그것으로 묻는다), 답변을 가공 없이 후속 메시지로 Advisor에게 회신. 다시 2.
   - `SKIP` → Advisor 루프 종료. Worker가 직접 처리하고 사용자에게 결과를 보고한다.
3. **구현.** 브리프대로 구현하고 완료 기준 명령을 실행한다. 브리프 범위를 벗어난 판단이 필요해지면 임의로 정하지 말고 후속 메시지로 Advisor에게 묻는다.
4. **완료 보고.** 후속 메시지로 Advisor에게 보고한다: 변경 파일 목록, 변경 요약, 완료 기준 실행 결과(명령과 출력 요지). 판정이 올 때까지 파일을 수정하지 않는다 (Advisor 검증 턴 — 동시 작성자 방지. Advisor의 사소한 마무리 수정과 충돌하지 않기 위함).
5. **판정 처리.**
   - `APPROVED` → 6으로.
...
- Advisor가 소실되거나 후속 메시지 전달이 실패하면(런타임에 서브에이전트 후속 메시지 기능이 없는 경우 포함), 지금까지의 결정·라운드 카운트를 요약해 새 Advisor를 스폰하고 루프를 이어간다.
