---
name: oneshot
description: 외부 에이전트 CLI(claude·codex)를 한 번 실행하고 stdout을 구조화해 받고 싶을 때 사용 — `claude -p` 수준의 raw 래퍼. 격리·커밋·반복·판정은 호출자가 정한다.
allowed-tools:
  - Read
...
외부 에이전트 CLI를 **한 번** 실행하는 raw 래퍼다. `claude -p`(또는 `codex exec`)에 벤더 중립 JSON 계약을 씌운 것 이상은 하지 않는다.
...
- 입력 — `prompt`(필수) · `cwd`(실행 디렉토리) · `system_prompt_file`(시스템 지침) · `vendor`(`claude` 기본 / `codex`)
