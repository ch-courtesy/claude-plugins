...
- `output`은 에이전트 stdout에서 후행 공백·개행(CR 포함)을 제거한 것 — 마지막 줄이 실제 마지막 내용이 된다. 다만 에이전트가 답을 코드펜스로 감싸면 마지막 줄은 펜스이므로, 표지 판정은 펜스·빈 줄을 걸러낸 뒤 본다.
...
- `agy`만 stdin을 받지 않아 프롬프트를 인자로 넘긴다 — 지침+프롬프트가 ARG_MAX(약 1MB)를 넘으면 이 벤더에서만 실행이 실패하고, 실행 중 내용이 `ps`에 노출된다. 큰 지침에는 claude·codex 를 쓴다.
- `prompt`가 비어 있어도 막지 않는다(입력 검증은 호출자 몫) — 빈 지시로 무인 에이전트가 뜬다.
