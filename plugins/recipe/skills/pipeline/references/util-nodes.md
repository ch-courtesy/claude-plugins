...
- id: ralph
  util: while
  skill: isolated-run                 # 항목마다가 아니라 회차마다 실행할 typed 스킬 (또는 inline: 정의)
  in: {task: $pipeline.task, repo: $pipeline.repo, commit: true}
  until: '.done'                      # jq boolean, 회차 출력 객체에 적용 — 참이면 정지
