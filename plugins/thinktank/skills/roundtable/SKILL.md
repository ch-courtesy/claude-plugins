...
`roundtable start <회의 설명>` / `roundtable resume <meeting-id>` / `roundtable status [meeting-id]` — 인자가 없으면 `start`로 간주한다. 회의 ID는 로컬 날짜와 주제 slug를 조합한 `YYYYMMDD-<slug>`다. 같은 ID의 회의가 있으면 임의 suffix를 만들지 말고 `resume` 또는 다른 제목을 선택받는다. 회의 산출물은 `.roundtable/<meeting-id>.md` **단일 파일 1개**다.

## 상태

`interviewing → agenda_approval → researching → research_review → roster_approval → discussing → documenting → completed | no_consensus`

회의 파일 템플릿은 `references/document-templates.md`를 따른다.

## 회의 파일 규율

- 상태 블록(`## 상태`)이 항상 회의 파일 최상단 첫 섹션이며, 모든 상태 전환 시 상태 블록을 먼저 갱신한다.
- 회의 시작 시 전체 뼈대를 한 번 만든 뒤에는 **섹션 단위로만 추가·갱신**하고 다른 섹션은 건드리지 않는다. 전체 파일 재작성을 금지한다.
- 기존 기록을 덮어써서 논의 계보, 기각된 대안, 반대 의견 근거를 지우거나 합의처럼 재작성하지 않는다.
- `resume`은 단일 회의 파일 하나만 읽고 기록된 상태와 다음 행동만 신뢰한다. 필수 산출물이 없거나 상태와 모순되면 추측해 복구하지 말고 불일치를 보고한다.
- `status`는 단일 회의 파일 하나만 읽어 보고한다. 파일을 생성·수정하거나 Agent를 호출하지 않는다.
...
- 모든 Agent brief는 자기완결적이어야 한다. 참여자·운영 역할 서브에이전트는 메인 컨텍스트를 보지 못하며 중첩 Agent 호출을 하지 않는다고 명시한다.
- 전체 자료를 모든 참여자에 반복 주입하지 않는다. 각 brief에는 공통 핵심 요약과 해당 역할·과업에 관련된 정보만 전달한다.
- 메인 세션이 Agent 호출과 최종 결정을 소유한다. 서브에이전트에게 회의 운영이나 최종 판단을 위임하지 않는다.

## 중앙 리서치 규범

- 조사는 중앙에서 한 번 수행해 참여자별 중복 조사를 방지한다.
