...
격리된 일회용 docker 컨테이너에서 새 Codex 인증 토큰을 발급받아 GitHub Actions 시크릿을 갱신한다. host `~/.codex` 시드는 건드리지 않는다.
...
격리가 필요한 이유: 그냥 `codex login`을 돌리면 host의 `~/.codex/auth.json`(다른 소비자가 쓰는 시드)을 덮어써 그 토큰까지 회전시킨다. 컨테이너로 격리하면 host 시드를 무접촉으로 둔 채 CI용 토큰만 새로 만든다.

## 절차

### 1. 전제 확인

`docker`·`gh`(secrets 쓰기 권한으로 인증됨)·`jq`가 필요하다. 대상 repo와 시크릿 이름을 정한다(기본: 현재 gh repo, `CODEX_AUTH_JSON`). 다르면 `RESEED_REPO=owner/name`·`RESEED_SECRET=<이름>`으로 지정한다.
...
### 2. 컨테이너 기동 + device-auth 시작

`references/reseed.sh setup`을 실행한다. 이 스크립트가 결정적으로 수행한다 — 격리 컨테이너 기동(host `~/.codex` 미마운트, `--network host`), codex CLI + `ca-certificates` 설치(codex 는 rustls 라 시스템 CA 스토어 필수 — node slim 이미지엔 없음), `codex login --device-auth` 시작. 출력된 **인증 URL과 일회용 코드**를 사용자에게 그대로 전달한다.

### 3. 사용자 인증 (대화형 게이트)

사용자에게 안내한다 — 브라우저에서 URL을 열고, 코드를 입력하고, **CI Codex 리뷰에 쓸 그 ChatGPT 계정**(기존 시크릿을 시드했던 것과 동일)으로 로그인. 코드는 약 15분 후 만료된다. 어느 계정을 쓸지 모호하면 `AskUserQuestion`으로 확인한다.

### 4. 완료 — 시크릿 갱신 + 정리

`references/reseed.sh complete`를 실행한다(인증 완료까지 폴링하므로 백그라운드 실행 권장). setup 에 `RESEED_REPO`를 줬다면 complete 에도 동일하게 준다. 스크립트가 auth.json을 검증(토큰 값 미출력)하고 `gh secret set`으로 시크릿을 갱신한 뒤 컨테이너를 파기한다. `SECRET_UPDATED_OK`가 성공 신호다. 실패·만료 시 `references/reseed.sh abort`로 정리하고 2단계부터 재시도한다.
...
- 토큰 값(auth.json 내용·access/refresh token)을 stdout·로그·PR에 출력하지 않는다. 스크립트는 umask 077 임시 파일로만 다루고 완료 후 shred한다.
- host `~/.codex/auth.json`을 컨테이너에 마운트하거나 수정하지 않는다(다른 소비자 시드 보호).
