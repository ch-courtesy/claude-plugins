...
- 실행: ⟨script: 정확한 명령과 stdin/stdout 방식 · http: curl 형태 · mcp: 도구 이름과 인자 매핑 · llm: 치환할 프롬프트 전문 + "outputs 스키마 JSON만 반환" 지시와 스키마, 서브에이전트 brief는 자기완결·중첩 Agent 금지 · kind: pipeline 참조: 해당 스킬을 서브에이전트로 실행하고 outputs JSON만 회수 · util: util-nodes 시맨틱을 해당 인스턴스 값으로 구체화한 지시⟩
- 출력 스키마: ⟨outputs⟩
