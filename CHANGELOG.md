...
### 변경 (호환)
- **fix 진단 절차에 레드플래그/합리화 차단 섹션 추가 (#543)** — `fix` 스킬의 정적 분석 진단 절차(`diagnosis.md`)가 "확정이 아닌 가설로 프레이밍" 원칙만 서술하고, 정적 증거 없이 가설을 성급히 확정하려는 흔한 생각 패턴을 구체적으로 차단하지 않았다. 가설 확정 충동·재현/실행 충동·후보 양자택일·시간 압박 4가지 레드플래그를 표로 추가하고, 각 대응을 기존 `[NEEDS CLARIFICATION]` 마커 규칙으로 귀결시켰다. 정적 분석 한정(실행·재현 금지) 원칙과 플러그인 자기완결(외부 스킬 doc-link 금지) 원칙은 그대로 유지했다.
